...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    ...
    if (isAuth !== "true" || role !== "manager") {
      ...
      // Initialize Dummy Data for new features if empty
  ...
  const handleTolakPesanan = (orderId: string) => {
    ...
    if (reason !== null) {
      ...
      if(orderToVoid) {
        ...
        // Simpan alasan pembatalan ke object order
      ...
      <aside className="w-[260px] bg-white flex flex-col h-full shrink-0 border-r border-slate-200 z-20">
        ...
        <div className="h-24 flex items-center px-8 border-b border-slate-100 shrink-0">
          ...
          <span className="font-extrabold text-slate-800 text-xl tracking-tight">
            Manager<span className="text-[#FF0055]">.</span>
          </span>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-6 overflow-y-auto scrollbar-hide">
          ...
          <div className="mt-8 mx-2 mb-4 p-5 rounded-2xl bg-rose-50 border border-rose-100 relative overflow-hidden">
             ...
             <div className="relative z-10">
               <p className="text-[10px] font-black text-[#FF0055] uppercase tracking-widest flex items-center gap-1.5 mb-3">
                 <RefreshCw size={12} className={countdown < 5 ? "animate-spin text-[#FF0055]" : ""} /> Live Void PIN
               </p>
        ...
        <header className="h-24 bg-white border-b border-slate-200 px-10 flex items-center justify-between shrink-0 shadow-sm z-10">
          <div>
            <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight capitalize">
               {activeTab === "dashboard" ? "Business Insight" : activeTab === "voids" ? "Live Antrean Kasir" : activeTab === "audit" ? "Audit Inventaris" : activeTab === "members" ? "Manajemen Pelanggan" : "Riwayat & Laporan"}
            </h1>
            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">
               Pantauan Operasional Terpadu
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                   ...
                   <div className="space-y-8">
                     <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col justify-between">
                        <div className="flex items-center gap-3 mb-6">
                           ...
                           <div>
                              <h3 className="font-bold text-slate-800">Performa Keuangan</h3>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Total Penjualan Bersih</p>
                           </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-slate-100 flex gap-10">
                           <div>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Total Sukses</p>
                              <p className="text-xl font-black text-slate-700 flex items-center gap-2"><Receipt size={16} className="text-emerald-500"/> {completedOrders.length} Struk</p>
                           </div>
                           <div>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Total Void</p>
                              <p className="text-xl font-black text-slate-700 flex items-center gap-2"><XCircle size={16} className="text-rose-500"/> {voidedOrders.length} Kasus</p>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex items-center gap-5">
                         ...
                         <div>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Kasir Shift Saat Ini (Elberth)</p>
                            <h3 className="text-xl font-black text-slate-800">{completedOrders.length + pendingOrders.length} <span className="text-sm font-bold text-slate-400">Transaksi dilayani</span></h3>
                         </div>
                     </div>
                   </div>

                   {/* Kanan: Produk Paling Laris */}
                   <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col">
                      <div className="flex items-center gap-3 mb-6">
                         ...
                         <div>
                            <h3 className="font-bold text-slate-800">Produk Paling Laris</h3>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Berdasarkan Total Terjual</p>
                         </div>
                      </div>
                         ...
                         {[...inventory].sort((a,b) => (b.sold || 0) - (a.sold || 0)).slice(0, 5).map((item, index) => (
                           <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all hover:border-[#FF0055] hover:shadow-sm">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <p className="font-bold text-sm text-slate-800 leading-tight mb-0.5">{item.name}</p>
                  ...
                  <div className="flex-1 overflow-y-auto">
                    {pendingOrders.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full text-slate-300 py-20">
                        <CheckCircle2 size={64} className="mb-4 opacity-30" strokeWidth={1.5} />
                        <p className="font-bold tracking-[0.2em] uppercase text-xs opacity-60">Tidak ada antrean kasir</p>
                      </div>
                    ) : (
                      <table className="w-full text-left">
                        <thead>
                           <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 border-b border-slate-100">
                              <th className="px-8 py-5">Waktu & ID</th>
                              <th className="px-8 py-5">Detail Pesanan</th>
                              <th className="px-8 py-5 text-right">Total Tagihan</th>
                              <th className="px-8 py-5 text-center">Tindakan Khusus</th>
                           </tr>
                        </thead>
                          ...
                          {pendingOrders.map(order => (
                            <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                              <td className="px-8 py-6">
                                 <span className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-rose-50 text-[#FF0055] border border-rose-100">{order.id}</span>
                                 <p className="text-[11px] font-semibold text-slate-500 mt-2 flex items-center gap-1.5"><Clock size={12}/> {order.time}</p>
                              </td>
                              <td className="px-8 py-6">
                                 <div className="space-y-1.5">
                                    {order.items.map((it: any, i: number) => (
                                       <p key={i} className="text-xs font-bold text-slate-700">{it.qty}x {it.name} <span className="text-slate-400 font-medium">({it.variant || 'Normal'})</span></p>
                                    ))}
                                 </div>
                              </td>
                              <td className="px-8 py-6 text-right font-black text-slate-800 text-lg">Rp {order.grandTotal.toLocaleString()}</td>
                              <td className="px-8 py-6 text-center">
                                 <button onClick={() => handleTolakPesanan(order.id)} className="px-4 py-2.5 bg-rose-50 text-[#FF0055] text-[11px] font-bold tracking-wider uppercase rounded-xl hover:bg-[#FF0055] hover:text-white transition-all shadow-sm border border-rose-100 hover:border-transparent flex items-center justify-center gap-2 mx-auto">
                                    <Trash2 size={14}/> Tolak / Void
                                 </button>
                              </td>
                            </tr>
                          ))}
            ...
            <div className="h-full p-10 overflow-y-auto animate-in fade-in bg-[#F4F7FA]">
              <div className="flex items-center justify-between mb-8">
                 ...
                 <div className="flex gap-3">
                    ...
                    <button onClick={() => showToast("success", "Export Laporan PDF", "Laporan siap dicetak.")} className="flex items-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-widest hover:bg-[#FF0055] transition-all shadow-md hover:-translate-y-0.5">
                       <FileDown size={16} /> Laporan PDF
                    </button>
                    <button onClick={() => showToast("success", "Export Excel", "Rekapitulasi Excel berhasil diunduh.")} className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-3 rounded-2xl text-[11px] font-bold uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-md hover:-translate-y-0.5">
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden h-[70vh] flex flex-col">
                {completedOrders.length === 0 && voidedOrders.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
                    <FileWarning size={64} className="mx-auto mb-6 opacity-30" strokeWidth={1.5} />
                    <p className="font-bold tracking-[0.2em] uppercase text-xs opacity-70">Belum ada riwayat terekam</p>
                  </div>
                ) : (
                     ...
                     <table className="w-full text-left">
                       <thead className="sticky top-0 bg-slate-50 border-b border-slate-100 z-10">
                          <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                             ...
                             <th className="px-8 py-5 text-right">Nilai Tagihan</th>
                             <th className="px-8 py-5">Status & Keterangan</th>
                          </tr>
                       </thead>
                       <tbody className="divide-y divide-slate-100">
                         {[...completedOrders, ...voidedOrders].map((order, idx) => (
                           <tr key={`${order.id}-${idx}`} className="hover:bg-slate-50/50 transition-colors">
                             <td className="px-8 py-6">
                                <span className="font-bold text-slate-800 text-[13px]">{order.id}</span>
                                <p className="text-[10px] font-semibold text-slate-400 mt-1">{order.time}</p>
                             </td>
                             <td className="px-8 py-6">
                                <span className="text-[13px] font-bold text-slate-700">{order.member ? order.member.name : "Umum"}</span>
                             </td>
                             ...
                             <td className="px-8 py-6">
                                <div className="flex flex-col items-start gap-1.5">
                                  {order.status === "completed" 
                                     ? <span className="bg-emerald-50 text-emerald-600 px-3.5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-emerald-100 inline-block">Sukses</span> 
                                     : <span className="bg-rose-50 text-[#FF0055] px-3.5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-rose-100 inline-block">Void</span>
                                  }
                                  {/* ALASAN VOID (BARU) */}
                                  {order.status === "voided" && order.voidReason && (
                                     <span className="text-[10px] font-bold text-rose-400">"{order.voidReason}"</span>
                                  )}
                                </div>
                             </td>
                           </tr>
               ...
               <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden h-[70vh] flex flex-col">
                  <div className="overflow-y-auto flex-1">
                     <table className="w-full text-left">
                       <thead className="sticky top-0 bg-slate-50 border-b border-slate-100 z-10">
                          <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                             <th className="px-8 py-5">Waktu & User</th>
                             <th className="px-8 py-5">Item Produk</th>
                             <th className="px-8 py-5 text-center">Tipe Mutasi</th>
                             <th className="px-8 py-5">Alasan / Keterangan</th>
                          </tr>
                       </thead>
                       <tbody className="divide-y divide-slate-100">
                         {stockLogs.map((log, idx) => (
                           <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                             <td className="px-8 py-5">
                                <span className="font-bold text-slate-800 text-[12px]">{log.time}</span>
                                <p className="text-[10px] font-semibold text-slate-500 mt-1 flex items-center gap-1"><UserCheck size={10}/> {log.user}</p>
                             </td>
                             <td className="px-8 py-5">
                                <span className="text-[13px] font-bold text-slate-800">{log.item}</span>
                             </td>
                             <td className="px-8 py-5 text-center">
                                {log.type === "masuk" 
                                  ? <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-3 py-1 rounded-lg uppercase tracking-wider">+{log.qty} Masuk</span>
                                  : <span className="text-[10px] font-bold text-rose-500 bg-rose-50 border border-rose-100 px-3 py-1 rounded-lg uppercase tracking-wider">-{log.qty} Keluar</span>
                                }
                             </td>
                             <td className="px-8 py-5 text-[11px] font-bold text-slate-500 italic">
                                "{log.reason}"
                             </td>
                           </tr>
                         ))}
            ...
            <div className="h-full p-10 overflow-y-auto animate-in fade-in bg-[#F4F7FA] flex gap-8">
               ...
               <div className="w-[360px] shrink-0">
                  <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                     ...
                     <form onSubmit={handleAddMember} className="space-y-5">
                        <div>
                           <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Nama Pelanggan</label>
                        ...
                        <div>
                           <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Diskon Default (%)</label>
               ...
               <div className="flex-1 bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                  <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white">
                     <h3 className="font-extrabold text-slate-800">Database Pelanggan</h3>
              ...
              <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in-95 border border-slate-100">
                 ...
                 <div className="space-y-3 mb-8">
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                       <p className="font-bold text-sm text-slate-800 mb-1">Otorisasi Void Gagal?</p>
                       <p className="text-xs text-slate-500 leading-relaxed">Pastikan kasir memasukkan "Live Void PIN" yang tertera di sidebar kiri Anda. PIN akan berganti setiap 60 detik.</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                       <p className="font-bold text-sm text-slate-800 mb-1">Laporan Excel Error?</p>
                       <p className="text-xs text-slate-500 leading-relaxed">Pastikan perangkat Anda terkoneksi internet. Jika masih terkendala, hubungi IT Support.</p>
                    </div>
                 </div>
